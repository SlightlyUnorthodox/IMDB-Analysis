...
Briefly, the Oblique tree was found to require far more memory than could be supplied by R when applied to the sample set, so no results can be shown for this method.

### RIPPER Results

```{r eval=TRUE,echo=FALSE}
print(ripCMMovie$overall)
summary(ripperPredictionsMovie)
```
...
summary(c45ModelPredictionsMovie)
```
...
{Cinematographer=['Campos, Ant\xc3\xb4nio']} => {Director=Antônio Campos}                   
{Director=Lionel Soukaz}                       => {Cinematographer=['Soukaz, Lionel']}         
...
Using the following list of genres, similar movies were broken up into 723 clusters (out of approximately 11K movies). This provided a reasonable means to provide a list of similar movies for any given movie.
...
Of the 29 attributes mentioned in earlier parts, only 12 attributes ended up being used for genre classification.
...
The measures we took to improve the results as much as possible include trial and error of which attribtues from the database yielded the greatest accuracy. Extreme care was also put into formatting and preprocessing the data so that the classification methods would correctly run, examples can be see in the methods and materials section. To improve the results, multiple trials could have improved results by looking at subsets of data that isolate certain areas of interest like only movies from 2010-2015. Furthermore, to check for the most ideal combination and number of attributes to be used for classifying, attributes were taken out and the same methods were run on the modified data set in order to see the weight and gravity of each attribute in determining their genre. Such trials were created as shown below:

```{r eval=TRUE,echo=FALSE}
#testing affect of director
trainingNoDirector <- training[,-3]
movieDataNoGenreNoDirector <- movieDataWOutGenre[,-3]
#C4.5 CLASSIFICATION
c45ModelMovie <- J48(Genre~., data = trainingNoDirector)
c45ModelPredictionsMovie <- predict(c45ModelMovie, movieDataNoGenreNoDirector)
c45CMMovie <- confusionMatrix(c45ModelPredictionsMovie, test$Genre)
```
In this example, the director attribute was removed in order to see if it would yield significant improvements on accuracy. Multiple trials of what is shown above were utilized to reach the ideal list of attributes used during classification.

**_Further Thoughts_**

Furhter analysis on the classification methods shows that of the methods utilized, Naive Bayes and Ripper yielded the most promising results. While knn yielded very inaccurate results and c4.5 seemed to be very biased towards the most dominant Genre in the set: "Drama". The code blurb shown above was utilized especially for c4.5 in order to yield some classifications other than "Drama" but no matter the attributes used, the movies were classified as "Drama". 


## Association Rules
**_The number of different roles you have in your dataset._**
The number of different roles involved for the usual casts apriori function is 7: Director, Producer, Writer, Actor1, Actor2, Actor3, and Actor4. These roles were chosen because more often than not, these individuals usually have preferences and tendencies to work together in the real world, so we aimed to show this relationship with our project. Other members we could have included were Cinematographer, Composer, and Costume Designer. The roles just mentioned are more specialized and often show very tight relationships with one anohter. 

Another way we could have chosen roles was to run multiple passes with different combinations of all the cast members mentioned above. For example, found association rules using only the attributes 'Composer', 'Director', and 'Costume Designer' would have been one pass. Information like htis would have been useful to show the different types of associations in the film industry. In the example just given, it is known that Tim Burton often works with the Composer Danny Elfman and has the same costume designer for many of his films. Other relationships like that have to exists so running multiple passes with specialized attribute sets could have led to more complete association rules. Additionally, the roles we chose were selected for the sake of getting the apriori algortihm to run in light of the issues we had. 
...
I ran apriori (the only way that R was allowing me to run) with no variables set for "lhs" and "rhs". We thought by doing the apriori this way would allow for all possibilities by not restricting those sides. We could have also utilized someting mentioned in the first question where we create a list of all possible subsets of attributes and run through each uniqe attribute list subset.
...
Furthermore, by increasing the total number of clusters, we are presented with the opportunity to potentially add in more attributes in order to create even more specialized clusters. With a smaller amount of clusters analyzing too many attributes at once can muddle the data because you are tyring to fit such a large amount of dimensionality into only a couple groups which will yeild more impure clusters.
